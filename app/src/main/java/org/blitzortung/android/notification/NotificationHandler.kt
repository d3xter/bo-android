...
class NotificationHandler(alertHandler: AlertHandler, sharedPreferences: SharedPreferences, private val context: Context): PreferenceChangedListener() {
    override fun onSharedPreferenceChanged(sharedPreferences: SharedPreferences, key: PreferenceKey) {
        val floatConverter = fun (x: String): Float? {
            try {
                return x.toFloat()
            } catch (e: NumberFormatException) {
                Log.e(Main.LOG_TAG, "bad distance limit format '$x'")
            }
        ...
        when(key) {
            PreferenceKey.ALERT_NOTIFICATION_DISTANCE_LIMIT -> notificationDistanceLimit = sharedPreferences.getAndConvert(key, "50", floatConverter) ?: 50f
            PreferenceKey.ALERT_SIGNALING_DISTANCE_LIMIT -> signalingDistanceLimit = sharedPreferences.getAndConvert(key, "25", floatConverter) ?: 25f
        }
    }
    ...
    init {
        ...
        onSharedPreferenceChanged(sharedPreferences, PreferenceKey.ALERT_NOTIFICATION_DISTANCE_LIMIT)
        onSharedPreferenceChanged(sharedPreferences, PreferenceKey.ALERT_SIGNALING_DISTANCE_LIMIT)
